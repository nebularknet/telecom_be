...
const AdminRegister = async (req, res) => {
  ...
  if (!req.body) {
    console.error(
      "Admin registration error: Request body is missing or empty."
    );
  ...
  if (!fullname || !email || !password) {
    ...
    console.warn("Admin registration validation failed: Missing fields.", {
      fullname: !!fullname,
      email: !!email,
      password: !!password,
    });
  ...
  try {
    ...
    if (existingUser) {
      ...
      console.warn(
        `Admin registration conflict: Email ${email} already exists.`
      );
      return res
        .status(409)
    ...
    console.info(
      `Admin registered successfully: ${newUser.email} (ID: ${newUser._id})`
    );
    res
      .status(201)
    ...
    if (error.code === 11000) {
      ...
      return res
        ...
        .json({
          message:
            "Admin with this email already exists (database constraint).",
        });
    }
